...
int connect_to_mirror(char *argv[]){
    // necessary variables
...
void check_connection_to_mirror(char *argv[]){
    // call function to get socket descriptor
    int connection_sd = connect_to_mirror(argv);
    if(connection_sd == -1){
        fprintf(stderr, "Could not connect to mirror. Please check IP/Port and try again.\n");
        exit(0);
    }
    else {
        printf("Connection to the mirror checked.\n");
        close(connection_sd);
    }
}

void route_command(char *argv[], int client_connection_sd){
    // connect to mirror and get socket descriptor
    int connection_sd = connect_to_mirror(argv);
    printf("Sucessfully Connected to the mirror: %d\n", connection_sd);
    ...
    while(keep_connection){
        ...
        char response_buffer[50000];
        // Null-terminate the received data
        command[bytes_received] = '\0';
        // send received command to mirror
        if (send(connection_sd, command, strlen(command), 0) < 0) {
            fprintf(stderr, "send() failed\n");
            exit(0);
        }
        printf("Command %s routed to Mirror\n\n", command);
        if(strcmp(command, "quit") == 0 || strcmp(command, " ") == 0 || strcmp(command, "") == 0){
        ...
        bytes_received = recv(connection_sd, response_buffer, 50000, 0);
        response_buffer[bytes_received] = '\0';
        if (bytes_received > 0){
            if(strcmp(response_buffer, "TAR_FILE_FLAG") == 0){
                // received tar file flag from mirror, forward it to client
                send(client_connection_sd, response_buffer, bytes_received, 0);
                ...
                long bytes_of_tar_file = atol(response_buffer);
                ...
                while (total_bytes_received < bytes_of_tar_file) {
                    bytes_received = recv(connection_sd, buffer_for_tar_file + total_bytes_received, bytes_of_tar_file, 0);
                    if (bytes_received <= 0) {
                        if (bytes_received == 0) {
                            printf("Error: The connection has been terminated by the server side.\n");
                        } else {
                ...
                printf("Response routed to the client.\n");
            }
            else if(strcmp(response_buffer, "MESSAGE_FLAG") == 0){
                ...
                send(client_connection_sd, response_buffer, bytes_received, 0);
                printf("Message routed to the client.\n");
            }
            else{
                fprintf(stderr, "Incompatible response received from mirror.\n");
            }
        }
    }
    exit(0);
...
void handle_client_at_mirror(char *argv[], int connection_sd){
    int pid = fork();
    if(pid == 0){
        route_command(argv, connection_sd);
    } 
    else if(pid == -1){
...
int main(int argc, char *argv[]){
    if(argc != 4){
        fprintf(stderr,"Please check structure: %s <Port#> <Mirror IP Address> <Mirror Port#>\n",argv[0]);
        exit(0);
    }
    // verify mirror is running or not
    check_connection_to_mirror(argv);
    ...
    while(1){
        ...
        if(connection_sd != -1){
            ...
            total_clients_handled++;
            if(total_clients_handled <= 6){
                printf("Server: count is %d\n", total_clients_handled);
                handle_client_at_server(connection_sd);
            }
            else if(total_clients_handled > 6 && total_clients_handled <= 12){
                printf("Mirror: count is %d\n", total_clients_handled);
                handle_client_at_mirror(argv, connection_sd);
            }
            else if(total_clients_handled %2 != 0){
                printf("Server: count is %d\n", total_clients_handled);
                handle_client_at_server(connection_sd);
            }
            else if(total_clients_handled %2 == 0){
